...
import { SHORT_PROBLEM, slugifyProblem } from "@/lib/problemSlugs";
    ...
    <div className="min-h-screen bg-surface-dark text-on-dark">
      <header className="px-6 md:px-10 pt-6 md:pt-8 flex items-baseline justify-between gap-6">
        <div className="min-w-0">
          <nav aria-label="Breadcrumb" className="flex items-center gap-3 text-[10px] uppercase tracking-[0.22em] text-on-dark/55 flex-wrap">
            ...
            <Link
              ...
              className="hover:text-on-dark transition-soft"
            >
              {shortLabel}
            </Link>
            <span aria-hidden className="text-on-dark/30">›</span>
            <span aria-current="page" className="text-on-dark">{community.toUpperCase()}</span>
          </nav>
          <h1
            className="mt-2 text-2xl md:text-3xl font-semibold tracking-tight text-on-dark"
          >
            {community} <span style={{ color: accent }}>· {shortLabel.toLowerCase()}</span>
          </h1>
          {narrative && (
            <p className="mt-2 text-[13px] leading-relaxed text-on-dark/80 max-w-[640px]">
              {narrative}
            </p>
          )}
        </div>
        <div className="shrink-0 flex flex-col items-end gap-2">
          ...
          <div className="text-right text-[10px] uppercase tracking-[0.22em] flex flex-col gap-1">
            <span className="num font-medium text-on-dark">
              {total} respondents
              {filterActive && (
                <span className="text-on-dark/40">
                  {" "}/ {baseline.total}
                </span>
              )}
            </span>
            ...
            {filterActive && (
              <span className="text-on-dark/55 normal-case tracking-normal font-medium">
      ...
      {total === 0 && (
        <div className="px-6 md:px-10 py-20 flex items-center justify-center text-on-dark/55 text-sm">
      ...
      {voices.length > 0 && (
        <section className="px-6 md:px-10 pt-2 pb-10">
          <div className="border-t border-on-dark/10 pt-6">
            <div className="flex items-baseline justify-between gap-4 mb-1">
              <div>
                ...
                <h2 className="mt-1 text-lg md:text-xl font-semibold tracking-tight">
                  ...
                  {filterActive && (
                    <span className="text-on-dark/65 font-normal">
                      {" · "}
                      {filter.gender && (
                ...
                <p className="mt-1 text-[12px] text-on-dark/55 max-w-[640px]">
                  {filterActive ? (
                    <>
                      Filtered to{" "}
              ...
              <div className="text-[10px] uppercase tracking-[0.22em] text-on-dark/45 shrink-0 text-right flex flex-col items-end gap-1">
                <div>
                  <span className="num">{filteredVoices.length}</span> shown
                  {filterActive && (
                    <span className="text-on-dark/35">
                      {" "}/ {voices.length} total
                    </span>
                  )}
                </div>
                ...
                {filterActive && (
                  <button
                    type="button"
                    onClick={clearFilter}
                    className="text-[10px] uppercase tracking-[0.22em] text-on-dark/55 hover:text-on-dark transition-soft mt-1"
                  >
                    × clear all
                  </button>
                )}
            ...
            {filterActive && filteredVoices.length === 0 && (
              <div className="mt-6 text-[12px] text-on-dark/55 italic">
      ...
      <footer className="px-6 md:px-10 py-6 border-t border-on-dark/10 text-[10px] uppercase tracking-[0.22em] text-on-dark/35 flex justify-between">
      ...
      >
        {(hovered || selected) && (
          <>
            <circle
              ...
              r={b.r + 12}
              fill="none"
              stroke={selected ? ACUTE_COLOR : "rgba(248,240,231,0.18)"}
            ...
            <circle
              ...
              r={b.r + 4}
              fill="none"
              stroke={selected ? ACUTE_COLOR : "rgba(248,240,231,0.45)"}
              strokeWidth={3}
              className="pointer-events-none"
            />
          </>
        ...
        {[0.78, 0.6, 0.42].map((scale, ringI) => (
          <circle
            ...
            fill="none"
            stroke={
              hovered ? "rgba(248,240,231,0.45)" : "rgba(248,240,231,0.18)"
            }
            strokeWidth={0.5}
            strokeDasharray="2 4"
        ...
        <text
          ...
          fontWeight={500}
          fill={hovered ? "rgba(248,240,231,0.95)" : "rgba(248,240,231,0.82)"}
          className="uppercase pointer-events-none"
          style={{
...
function VoiceCard({
  ...
  return (
    <article
      className="relative rounded-md border border-on-dark/10 bg-surface-dark-sunken p-4 pl-5 overflow-hidden"
    >
      ...
      />
      <div className="flex items-center justify-between gap-2 text-[10px] uppercase tracking-[0.22em] text-on-dark/50">
        <span>
          {voice.gender || "—"} · {voice.ageBand}
        </span>
        {voice.acute && (
          <span
            className="px-1.5 py-0.5 rounded-sm text-[9px] font-medium tracking-[0.22em]"
            style={{
      ...
      {voice.occupation && voice.occupation !== "Unknown" && (
        <div className="mt-1 text-[11px] text-on-dark/65">
          <span className="text-on-dark/40">Occupation · </span>
          {voice.occupation}
        </div>
      )}
      <blockquote className="mt-2 text-[13px] leading-relaxed text-on-dark/85 italic">
        &ldquo;{voice.whyUrgent}&rdquo;
      </blockquote>
      <div className="mt-3 pt-3 border-t border-on-dark/10">
        <div className="text-[9px] uppercase tracking-[0.22em] text-on-dark/45">
          {voice.acute ? "Their top priority" : "Among their concerns"}
        </div>
        <div className="mt-1 text-[12px] text-on-dark/75">{canonical}</div>
      </div>
    </article>
  );
}
