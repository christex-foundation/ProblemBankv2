...
const LENSES: Lens[] = [
  { id: "all", title: "Every voice, all at once", matcher: null },
  {
    id: "water",
    title: "Water tops what's broken",
    matcher: (n) => {
  ...
  {
    id: "jobs",
    title: "When jobs are the loudest cry",
    matcher: (n) => {
      const t = n.topPriorityText.toLowerCase();
  ...
  {
    id: "youth",
    title: "What the young name first",
    matcher: (n) => n.ageBand === "18-25",
  },
  {
    id: "women",
    title: "What women say is broken",
    matcher: (n) => n.gender === "Female",
  },
  {
    id: "skills",
    title: "Asking for skills, not aid",
    matcher: (n) => n.topSupport === "Skills training",
  },
];

export function Scrolly({ universe }: ScrollyProps) {
  ...
  return (
    ...
    >
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        <header className="absolute top-6 md:top-8 left-6 md:left-10 right-6 md:right-10 flex items-baseline justify-between gap-6 z-10">
          <div className="min-w-0">
            ...
            <h1 className="mt-1.5 text-[15px] md:text-base font-semibold tracking-[-0.005em] text-foreground leading-tight max-w-[42ch]">
              What {totalRespondents} Sierra Leoneans say is broken
            </h1>
          </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center px-6 md:px-10 pt-24 md:pt-28 pb-24 md:pb-28">
          <div className="flex items-end gap-6 md:gap-10">
            <div
              className="aspect-[14/9] flex-shrink-0"
              style={{
                width:
                  "min(72vw, 1100px, calc((100vh - 18rem) * 14 / 9))",
              }}
            >
              <Universe universe={universe} focus={activeFocus} searchQuery={query} />
            </div>
            <nav
              aria-label="Lenses"
              className="flex flex-col gap-2.5 items-end pb-1"
            >
              {LENSES.map((lens, i) => {
  ...
  return (
    <div className="flex items-center gap-2 bg-paper/95 backdrop-blur border border-foreground/15 px-4 py-2.5 text-sm shadow-sm">
      ...
      <input
        ...
        placeholder="Search community, region, problem…"
