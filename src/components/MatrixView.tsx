    ...
    <div className="min-h-screen bg-surface-dark text-on-dark relative overflow-hidden">
      ...
      <header className="absolute top-6 md:top-8 left-6 md:left-10 right-6 md:right-10 z-20">
        <div className="flex items-baseline justify-between gap-6">
          <div>
            <div className="text-[10px] uppercase tracking-[0.22em] text-on-dark/55">
          ...
          {dimension === "problem" && viewMode === "matrix" && (
            <>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-[10px] uppercase tracking-[0.22em] text-on-dark/55">
      ...
      >
        ...
        <svg
          ...
          role="img"
          aria-label={`Bubble chart of ${bubbles.length} problems, each sized by the number of respondents who flagged it.`}
        >
          {bubbles.map((b, i) => {
      ...
      <div className="absolute bottom-6 md:bottom-10 left-6 md:left-10 z-20">
        ...
        <div className="text-[10px] uppercase tracking-[0.22em] text-on-dark/45 mt-1">
          total respondents
        </div>
      </div>
...
function FilterGroupRow({
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-[10px] uppercase tracking-[0.22em] text-on-dark/55 font-medium">
        {label}
      </span>
...
function FilterChip({
  ...
  return (
    <button
      ...
      className={
        active
          ? "px-3 py-1.5 text-[11px] font-medium bg-on-dark/10 text-on-dark rounded text-left"
          : "px-3 py-1.5 text-[11px] text-on-dark/45 hover:text-on-dark/75 transition-soft text-left"
      }
    >
      {children}
    </button>
...
function HoverCard({
  ...
  return (
    ...
    >
      <div className="bg-surface-dark-raised border border-on-dark/15 rounded-md px-4 py-3 w-[230px] shadow-xl">
        <div className="text-[10px] uppercase tracking-[0.22em] text-on-dark/45 font-medium">
          {dimension === "problem" ? "Top communities" : "Top problems"}
        </div>
        <div className="num text-3xl font-light mt-1 leading-none">
          {bubble.count}
        </div>
        <div className="text-[10px] uppercase tracking-[0.16em] text-on-dark/55 mt-1">
          {bubble.label}
        </div>
        <div className="mt-3 pt-2 border-t border-on-dark/10 flex flex-col gap-1.5">
          {detail.map(([name, n]) => (
            <div
              key={name}
              className="grid grid-cols-[1fr_auto] items-baseline gap-3 text-[11px]"
            >
              <span className="text-on-dark/75 truncate">{name}</span>
              <span className="num text-on-dark/95 font-medium">{n}</span>
            </div>
          ))}
        </div>
      </div>
