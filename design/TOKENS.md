...
|---|---|---|---|---|
| `color.background` | `#F8F0E7` | `--background` | `bg-background` | Page background. Warm pale beige, the default canvas. |
| `color.paper` | `#faf8f3` | `--paper` | `bg-paper` | Floating panels: detail-page sections (`tone="paper"`), dropdown menus, document tab panel. Marginally brighter than background. |
| `color.shelf` | `#F1E7DB` | `--shelf` | `bg-shelf` | Library shelf cards (`/library` entry tiles). Slightly warmer/darker than background so cards recess into the page rather than float above it. |
...
`html { overflow-y: scroll; }` is set unconditionally in `globals.css` so the vertical scrollbar gutter is always reserved. This avoids a ~15px horizontal layout shift ("the shake") when navigating between a tall, scrollable page and a shorter one. The cost is a permanently visible (but inert when not scrolling) scrollbar on platforms that render one.
